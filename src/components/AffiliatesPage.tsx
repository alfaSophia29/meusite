...
import { User, AffiliateSale, Product, UserType, Store, Page } from '../types';
import { getSalesByAffiliateId, getProducts, getUsers, getSalesByStoreId, getStores, findUserById, findProductById, saveAffiliateLink, getAffiliateLinks } from '../services/storageService';
import { ChartBarIcon, ClipboardDocumentCheckIcon, LinkIcon, PresentationChartLineIcon, ShoppingBagIcon, SparklesIcon, TrophyIcon, MagnifyingGlassIcon, ArrowTopRightOnSquareIcon } from '@heroicons/react/24/outline';
...
const AffiliatesPage: React.FC<AffiliatesPageProps> = ({ currentUser, onNavigate }) => {
  ...
  const handleGenerateLink = async (productId: string) => {
    ...
    await saveAffiliateLink(currentUser.id, productId, link, product.userId);
    await loadData(); // Recarrega para mostrar nos "Meus Links"
                    ...
                    <div key={product.id} className="bg-white dark:bg-darkcard rounded-[2.5rem] shadow-lg border border-gray-100 dark:border-white/5 overflow-hidden group hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div className="p-6">
                            ...
                            >
                                ...
                                {copiedLink === product.id ? 'Copiado!' : 'Gerar Link'}
                            </button>
                        </div>
                    </div>
                ))}
      ...
      {activeTab === 'links' && (
        <div className="bg-white dark:bg-darkcard rounded-[3rem] border border-gray-50 dark:border-white/5 overflow-hidden shadow-xl animate-fade-in">
            <div className="p-8 border-b border-gray-50 dark:border-white/5">
                <h3 className="font-black text-lg text-gray-900 dark:text-white uppercase tracking-tighter">Meus Links Gerados</h3>
            </div>
            {myLinks.length > 0 ? (
                <div className="divide-y divide-gray-50 dark:divide-white/5">
                    {myLinks.map(link => (
                        <div key={link.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-gray-50 dark:hover:bg-white/5">
                            <div className="flex items-center gap-4">
                                <div className="w-16 h-16 rounded-2xl bg-gray-100 dark:bg-white/5 shrink-0 overflow-hidden">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <span className="text-[9px] text-gray-400 font-bold uppercase">{new Date(link.timestamp).toLocaleDateString()}</span>
                                    </div>
                                </div>
                            </div>
