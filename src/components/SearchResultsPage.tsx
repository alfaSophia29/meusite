...
import { User, Post, Product, ProductType, UserType, Page } from '../types';
  ...
  return (
    <div className="bg-white dark:bg-darkcard p-6 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-white/5 flex flex-col items-center text-center transition-all hover:shadow-xl hover:-translate-y-1 w-full max-w-[320px]">
      ...
      <p className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-4">
        {user.userType === 'CREATOR' ? 'Conta Profissional' : 'Membro'}
      </p>
  ...
  >
    <div className="relative h-44 overflow-hidden">
      ...
      <div className="absolute top-3 left-3 bg-white/90 dark:bg-darkcard/90 backdrop-blur-md px-2 py-1 rounded-lg shadow-sm border border-black/5">
        <span className="text-[8px] font-black uppercase text-blue-600">{product.type === 'PHYSICAL' ? 'Físico' : 'Digital'}</span>
      </div>
    </div>
    <div className="p-5">
...
const SearchResultsPage: React.FC<SearchResultsPageProps> = ({ currentUser, query, onNavigate, refreshUser }) => {
  ...
  const performSearch = useCallback(async () => {
    ...
    // 🍪 Gravar Cookie de Rastreamento de Busca para Personalização do Feed
    if (typeof document !== 'undefined') {
      document.cookie = `search_interest=${encodeURIComponent(lowerQuery.trim())}; path=/; max-age=604800; SameSite=Lax`;
      
      let currentHistory: string[] = [];
      const match = document.cookie.match(new RegExp('(^| )search_history=([^;]+)'));
      if (match) {
        currentHistory = decodeURIComponent(match[2]).split(',').filter(Boolean);
      }
    ...
    if (onlyVerified) usersList = usersList.filter(u => u.userType === 'CREATOR');

    const postsRes = await getPosts(undefined, 200);
    const allPosts = postsRes.items;
    let postsList = allPosts.filter(p => p.content?.toLowerCase().includes(lowerQuery));
    if (sortBy === 'newest') postsList = [...postsList].sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));

    const productsRes = await getProducts(200);
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto px-4 md:px-8 py-8 md:py-12 pt-16">
      ...
      <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-100 dark:border-white/5 pb-10">
        <div>
           <p className="text-blue-600 font-black uppercase tracking-[0.3em] text-[10px] mb-2">Central de Busca CyBer</p>
        ...
        <main className="lg:col-span-9 animate-fade-in">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-32">
               ...
               <p className="text-gray-400 font-black uppercase tracking-widest text-[10px]">Vasculhando a FacePhone...</p>
            </div>
