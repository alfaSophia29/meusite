...
import { findUserById, createTransaction } from '../services/storageService';
...
const MonetizationDashboard: React.FC<Props> = ({ user, onBack }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'revenue' | 'eligibility'>('overview');
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (success) {
        alert("Solicitação de saque enviada com sucesso! O valor será creditado em sua conta em até 5 dias úteis.");
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {isMonetized && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <div className="bg-white dark:bg-darkcard p-6 rounded-[2rem] border dark:border-white/10 flex items-center gap-4">
              <div className="p-4 bg-blue-50 dark:bg-blue-500/10 rounded-2xl text-blue-500">
                 <MegaphoneIcon className="h-6 w-6" />
              </div>
              <div>
                 <p className="text-[10px] font-black text-gray-500 uppercase">Receita de Anúncios</p>
                 <p className="text-xl font-black dark:text-white">${statsBreakdown.ads.toFixed(2)}</p>
              </div>
           </div>
           <div className="bg-white dark:bg-darkcard p-6 rounded-[2rem] border dark:border-white/10 flex items-center gap-4">
              ...
              <div>
                 <p className="text-[10px] font-black text-gray-500 uppercase">Super Chats & Gifts</p>
                 <p className="text-xl font-black dark:text-white">${statsBreakdown.donations.toFixed(2)}</p>
              </div>
           </div>
           <div className="bg-white dark:bg-darkcard p-6 rounded-[2rem] border dark:border-white/10 flex items-center gap-4">
              <div className="p-4 bg-purple-50 dark:bg-purple-500/10 rounded-2xl text-purple-500">
                 <UserGroupIcon className="h-6 w-6" />
              </div>
              <div>
                 <p className="text-[10px] font-black text-gray-500 uppercase">Premium Subs Share</p>
                 <p className="text-xl font-black dark:text-white">${statsBreakdown.subs.toFixed(2)}</p>
              </div>
           </div>
        </div>
      )}
      ...
      {!isMonetized ? (
        ...
        /* Monetized Stats Dashboard */
        <div className="space-y-8">
          {/* Main Metrics */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { label: 'Ganhos Estimados', value: `$${totalEarnings.toFixed(2)}`, icon: CurrencyDollarIcon, color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-500/10' },
              { label: 'Receita de Anúncios', value: `$${adRevenue.toFixed(2)}`, icon: GlobeAltIcon, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-500/10' },
              ...
              <div key={i} className="bg-white dark:bg-darkcard p-6 rounded-[2rem] border dark:border-white/10 shadow-sm">
                ...
                <p className="text-2xl font-black dark:text-white mt-1">{stat.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white dark:bg-darkcard p-8 rounded-[2.5rem] border dark:border-white/10 shadow-sm">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-xl font-black dark:text-white uppercase tracking-widest text-[12px]">Receita Diária ($)</h3>
                <div className="flex gap-2">
                  <button className="px-4 py-2 bg-brand/10 text-brand rounded-full text-xs font-black">30 Dias</button>
                  <button className="px-4 py-2 bg-gray-50 dark:bg-white/5 text-gray-400 rounded-full text-xs font-bold">7 Dias</button>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-darkcard p-8 rounded-[2.5rem] border dark:border-white/10 shadow-sm">
              ...
              <div className="space-y-6">
                {[
                  { label: 'Video Ads', amount: adRevenue, percent: (adRevenue / totalEarnings || 0) * 100, color: 'bg-blue-500' },
                  { label: 'Super Chat', amount: donationRevenue, percent: (donationRevenue / totalEarnings || 0) * 100, color: 'bg-orange-500' },
                  { label: 'Membros', amount: totalEarnings - adRevenue - donationRevenue, percent: ((totalEarnings - adRevenue - donationRevenue) / totalEarnings || 0) * 100, color: 'bg-purple-500' },
                ].map((source, i) => (
              ...
              <div className="mt-12 p-6 bg-brand/5 border border-brand/20 rounded-[2rem]">
                <p className="text-[10px] font-black text-brand uppercase tracking-widest text-center mb-2">Próximo Pagamento</p>
                <p className="text-2xl font-black text-brand text-center">15 Mai</p>
                <p className="text-[9px] font-bold text-gray-400 text-center mt-2">Mínimo de $100.00 para resgate</p>
              </div>
            </div>
          </div>
        </div>
